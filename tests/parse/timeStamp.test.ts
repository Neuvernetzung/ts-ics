...
it("Test Ics Timestamp Parse - VTimezones - standard", async () => {
	const timestamp = `DTSTART;TZID=Fictive/Timezone:20230110T140000`;
...
it("Test Ics Timestamp Parse - VTimezones - outlook", async () => {
	const timestamp = `DTSTART;TZID=Mountain Standard Time:20230110T140000`;

	const timezoneString = readFileSync(
		`${__dirname}/fixtures/outlookTimezone.ics`,
		"utf8"
	);
	const timezoneStrings = [...timezoneString.matchAll(getTimezoneRegex)].map(
		(match) => match[0]
	...
	expect(parsed.date.getUTCHours()).toEqual(21);
