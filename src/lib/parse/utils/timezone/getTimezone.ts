...
import { getOffsetMillis, parseOffset, UtcOffset } from "./offset";
...
export const getTimezoneObjectOffset = (
  ...
  if (vTimezone) {
    ...
    if (utcOffset) {
      ...
      return {
        offset: `${!utcOffset.positive ? "-" : "+"}${utcOffset.hourOffset}${
...
const getSortedProps = (vTimezone: VTimezone): VTimezoneProp[] =>
  vTimezone.props.sort((a, b) => compareAsc(a.start, b.start));

const getOffset = (date: Date, vTimezone: VTimezone): UtcOffset | undefined => {
  let closestIndex = -1;
  let closest: VTimezoneProp | undefined;
  let closestValue: Date | undefined;
  ...
  for (let i = 0; i < sortedProps.length; i += 1) {
    ...
    if (!!dateValue && (!closestValue || closestValue < dateValue)) {
  ...
  let observanceAfter: VTimezoneProp | undefined;
  let observanceAfterStart: Date | undefined;
  ...
  const boundary = {
    observanceInStart,
    observanceIn,
    observanceAfterStart,
    observanceAfter,
  };

  const offset = boundary.observanceIn?.offsetTo;
...
const getObservanceDateClosestToTheGivenDate = (
  ...
  let cur: Date | undefined;
  let stopped = false;
  const it = createIterator(observance);
  while (it.hasNext()) {
    cur = it.next();
