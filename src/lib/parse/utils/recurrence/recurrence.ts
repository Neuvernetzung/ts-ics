...
export function createRecurrenceIterator(
	...
	let monthGenerator: DateGenerator | undefined = undefined;
	let dayGenerator: DateGenerator | undefined = undefined;
	let secondGenerator: DateGenerator | undefined = undefined;
	let minuteGenerator: DateGenerator | undefined = undefined;
	let hourGenerator: DateGenerator | undefined = undefined;
	...
	switch (freq) {
		...
		case "MONTHLY":
			if (byMonthday.length > 0) {
				...
				if (freq === "YEARLY") {
					monthGenerator = getByMonthGenerator(
						[getMonth(dtStart)],
						start
					);
				}
				dayGenerator = getByMonthDateGenerator(
					[getDate(dtStart)],
					start
				);
			}
