import { WeekDay, WeekDayOrdinal, WeekdayNumberObject } from "@/types";
import {
	daysInYear,
	differenceInDays,
	getDate,
...
export const byMonthDayFilter = (monthDays: number[]): DatePredicate => {
	return (date) => {
		let nDays = getDaysInMonth(date);
...
export const untilCondition = (until: Date): DatePredicate => {
