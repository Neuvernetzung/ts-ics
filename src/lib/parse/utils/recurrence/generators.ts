...
export const getSerialMonthGenerator: SerialGeneratorFn = (
	...
	let start = sub(dtStart, { months: interval });
...
export const getSerialDayGenerator: SerialGeneratorFn = (interval, dtStart) => {
	let start = sub(dtStart, { days: interval });
	...
	return (builder) => {
		...
		if (year == builder.year && month == builder.month) {
			...
			if (interval != 1) {
				 ...
				 */
				let daysBetween = differenceInDays(
...
export const getSerialHourGenerator: SerialGeneratorFn = (
	...
	let start = sub(dtStart, { hours: interval });
...
export const getByMonthDateGenerator: ByGeneratorFn = (dates, dtStart) => {
	...
	const convertDatesToAbsolute = () => {
		...
		let nDays = getDaysInMonth(new Date(year, month, 1));
		for (let j = 0; j < udates.length; ++j) {
			let date = udates[j];
			if (date < 0) {
				date += nDays + 1;
			}
...
export const getByDayGenerator = (
	...
	const generateDates = () => {
		let nDays: number;
		let dow0: WeekDay;
		let nDaysInMonth = getDaysInMonth(new Date(year, month, 1));
...
export const getByYearDayGenerator = (
	...
	const checkMonth = () => {
		...
		let doyOfMonth1 = getDayOfTheYear(year, month, 1);
		let nDays = getMonthLength(year, month);
		let nYearDays = getYearLength(year);
		const udates = new Set<number>();
...
export const bySetPosInstanceGenerator = (
	...
	let maxPos = uSetPos[uSetPos.length - 1];
	let allPositive = uSetPos[0] > 0;

	let pushback: Date | undefined = undefined;
	...
	return (builder) => {
		...
		while (!candidates || i >= candidates.length) {
			 ...
			 */
			let d0: Date | undefined = undefined;
			if (!!pushback) {
				d0 = pushback;
			...
			while (limit > dates.length) {
				...
				if (!d0) {
					...
					switch (freq) {
						...
						case "MONTHLY":
							contained =
								getMonth(d0) == getMonth(d) &&
								getYear(d0) == getYear(d);
							break;
						case "YEARLY":
