...
>(
  ...
  rawString: string,
  options: ConvertIcsComponentProps<TData, TNonStandard>
): TData => {
  ...
  const lineStrings = splitLines(
    Object.values<{
      icsComponent: IcsComponent;
    }>(options.childComponents || {}).reduce(
      (prevString, { icsComponent }) =>
        prevString.replace(createGetRegex(icsComponent), ""),
      cleanedFileString
    )
  );
    ...
    (Object.keys(childComponents) as (keyof typeof childComponents)[]).forEach(
      (childComponentKey) => {
        ...
        if (childComponentFunction) {
          const childStrings = [
            ...cleanedFileString.matchAll(
              createGetRegex(childComponentFunction.icsComponent)
            ),
          ].map((match) => match[0]);

          const childData = childStrings
            .map((childString) => childComponentFunction.convert(childString))
            .filter((v) => v);
